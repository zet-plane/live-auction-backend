...
| WebSocket Handler | `internal/app/ws/handler/ws.go` | 校验 ticket、Redis `GETDEL`、升级连接、创建 Conn |
...
- `CheckOrigin` 当前返回 true。
- `Hub.Register` 将连接加入 `rooms[roomID][connID]` 和 `users[userID]` 两个内存索引。
- `Hub.Remove` 从房间索引和用户索引移除连接；房间无连接时删除房间索引。
- Hub 有 Redis client 时，注册连接会异步执行 `SADD auction:room:{room_id}:online_users user_id` 和 `HINCRBY auction:room:{room_id}:state online_count 1`。
- Hub 有 Redis client 时，移除连接会异步执行 `SREM auction:room:{room_id}:online_users user_id` 和 `HINCRBY auction:room:{room_id}:state online_count -1`。
...
- `online_count` 当前按连接增减，`online_users` Set 按唯一用户存储；同一用户多连接时二者可能不同。
...
| 字段 | 来源 | 规则 | 涉及接口 / 方法 | 测试点 ID |
| --- | --- | --- | --- | --- |
| `auction:room:{room_id}:state.online_count` | Redis HASH | Register 时 `+1`，Remove 时 `-1`；当前按连接数变化 | `syncRedisOnJoin`、`syncRedisOnLeave` | `WS.FIELD.online_count.*` |
| `auction:room:{room_id}:online_users` | Redis SET | Register 时 SADD user_id；Remove 时 SREM user_id；当前按唯一用户集合存储 | `syncRedisOnJoin`、`syncRedisOnLeave` | `WS.FIELD.online_users.*` |
...
- Redis `online_count` 和 `online_users` 按当前实现发生对应变化。
- 跨房间连接不能收到本房间广播。
...
- 同一用户多连接时 users 索引保留所有连接，移除其中一个不影响另一个。
- Redis 为 nil 时 Register / Remove 不 panic。

#### 集成测试点

- Register 后 Redis Set 包含 user_id，HASH `online_count` 加 1。
- Remove 后 Redis Set 移除 user_id，HASH `online_count` 减 1。
...
- 客户端收到对应 `auction_started`、`bid_success`、`user_outbid`、`auction_extended`、`auction_ended`、`auction_cancelled`、`order_created` 中适用的事件。
...
| Redis 在线状态并发增减 | 是 | 测试 Redis | online state 不出现不可解释的负数；差异需按“连接数 vs 唯一用户”规则解释 |
...
| Redis `auction:room:{room_id}:state.online_count` | Register / Remove | `HGET online_count`，按连接数变化验证 | 删除本批次 room state 或恢复初始值 |
...
| 在线人数和在线用户集合不一致 | 集成 / 并发 | 同一用户多连接或异常断开 | 按确认后的规则验证并记录 Redis 证据 |
...
- `online_count` 的业务口径应是“连接数”还是“唯一用户数”？当前代码按连接数 HINCRBY，但 `online_users` 是唯一用户 Set。
