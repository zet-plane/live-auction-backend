...
- 目标并发用户数下，客户端按接口 P50 / P95 / P99 和服务侧按 route P50 / P95 / P99 是否同时达标。
...
每档压测结果必须先给总览，再给核心接口拆分。总览表只用于阶段概览，不得替代单接口结论：

| 阶段 | 时间窗口 | 目标 QPS | 实际 QPS | 并发 | 总数 | 成功 | HTTP 失败率 | 业务失败率 | 超时率 | Client E2E P50 | Client E2E P95 | Client E2E P99 | 状态码 | 业务码 | CPU | MySQL 摘要 | Redis 摘要 | 对账 | 结论 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| smoke |  | 10 |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |

核心接口拆分表必须包含所有参与 request mix 的业务接口，例如出价、排行榜、商品详情、房间详情：

| 阶段 | 接口 | 请求占比 | Client E2E P95 | Client E2E P99 | Server route P95 | Server route P99 | RPS | HTTP 失败 | 超时 | 业务失败 | 状态码 | 业务码 | 结论 |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- | --- | --- |
| public_300 | bid | 80% |  |  |  |  |  |  |  |  |  |  |  |

最低统计指标包括：目标 QPS、实际 QPS、并发、总请求数、成功数、HTTP 失败数、业务失败数、超时数、错误率、超时率、业务失败率、客户端端到端 P50/P95/P99/max、按接口客户端端到端 P50/P95/P99/max、服务侧按 route P50/P95/P99 或至少 P95/P99、HTTP 状态码分布和业务码分布。P50 / P95 / P99 是必要指标，但不足以单独支撑压测结论。

禁止只用全局服务侧 HTTP P95/P99 代表业务接口性能。全局服务侧 histogram 可作为背景信号；验收结论必须优先引用和 request mix 对齐的 per-route / per-interface 指标。
