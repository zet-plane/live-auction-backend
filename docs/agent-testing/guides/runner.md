...
已批准计划进入执行阶段后使用 subagent 并行测试多个目标：
读取 docs/agent-testing/README.md
读取 docs/agent-testing/templates/protocol.md
读取 docs/agent-testing/guides/runner.md
读取 docs/agent-testing/guides/subagent.md
...
Agent 不允许通过猜测补齐这些信息。
Agent 也不允许把未确认语义写入正式测试计划；正式计划只能包含已确认的预期和不变量。

### 3. 生成测试计划

执行前，agent 必须输出测试计划。按 `templates/protocol.md` 的通用字段输出，concurrency、performance 追加各自专项字段。
...
测试计划批准后、执行前，agent 应提醒用户选择执行编排方式：主 agent 串行、subagent 串行或 subagent 并行。subagent 分工、可读取上下文、真实依赖授权和子批次隔离属于执行器派发信息，不写入业务测试计划正文。
...
| 性能压测 | `guides/performance/README.md`、`guides/performance/types.md`、`guides/performance/online.md`、`guides/performance/runner.md`、`guides/environment.md`、目标模块或流程文档 | 明确压测模型、阈值、压测源、停止条件、监控指标、runner 代码资产和业务抽样对账 |
...
- 如果是性能压测，是否已读取 `guides/performance/README.md`、`guides/performance/types.md`、`guides/performance/online.md`、`guides/performance/runner.md` 和 `guides/environment.md`？
...
- 是否发现需要补充测试契约文档的地方？
- 如果使用了 subagent，是否已在复核输出后关闭不再需要的 subagent？
