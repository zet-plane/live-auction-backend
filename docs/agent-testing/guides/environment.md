...
本文档定义 agent 执行测试前如何准备环境、依赖、服务入口和测试数据。
...
执行前必须先判定本次测试目标和测试类型。环境准备不能脱离测试目标单独进行。

最低判定顺序：

1. 明确要测试的模块、流程、接口或服务入口。
2. 判断本次测试是否面向线上服务、线上等价服务、本地服务或纯单元逻辑。
3. 再按测试类型选择依赖策略、服务可达性检查、测试数据和清理方式。
...
| Agent 性能压测 | 允许 | 使用准生产、线上等价或用户批准的线上服务；必须先确认被测服务入口可达，再隔离测试数据、记录监控指标和停止条件 |
...
当本次测试面向线上服务或线上等价服务时，agent 必须先确认“要测什么”和“线上服务是否通”。

检查顺序：

1. 明确被测目标：模块、流程、接口路径、WebSocket 路径或服务入口。
2. 明确线上入口：Ingress、网关、Service、port-forward 或用户批准的访问方式。
3. 在授权范围内执行只读可达性探测，例如健康检查、版本接口、只读查询接口或 WebSocket 握手。
4. 记录探测结果、时间窗口和脱敏入口摘要。
5. 只有线上入口可达，才继续准备测试数据、runner、并发请求或压测流量。

如果线上服务不通，agent 必须停止真实依赖测试并输出环境阻塞；不得继续创建测试数据、发起写请求、执行 smoke、启动压测工具或把结果写成业务失败。
...
- 如果测试面向线上服务，线上入口已在授权范围内完成只读可达性探测。
