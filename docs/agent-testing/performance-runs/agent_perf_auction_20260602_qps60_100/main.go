...
func loadConfig() Config {
	batchID := getenv("PERF_BATCH_ID", "agent_perf_auction_20260602_qps60_100")
	stages := filterStages([]StageConfig{
		{Name: "smoke_10qps_20ws", TargetQPS: 10, TargetWS: 20, Concurrency: 10, Duration: 1 * time.Minute, RequestMix: "auction_mix"},
		{Name: "step_60qps_160ws", TargetQPS: 60, TargetWS: 160, Concurrency: 60, Duration: 3 * time.Minute, RequestMix: "auction_mix"},
		{Name: "step_70qps_160ws", TargetQPS: 70, TargetWS: 160, Concurrency: 70, Duration: 3 * time.Minute, RequestMix: "auction_mix"},
		{Name: "step_80qps_160ws", TargetQPS: 80, TargetWS: 160, Concurrency: 80, Duration: 3 * time.Minute, RequestMix: "auction_mix"},
		{Name: "step_90qps_160ws", TargetQPS: 90, TargetWS: 160, Concurrency: 90, Duration: 3 * time.Minute, RequestMix: "auction_mix"},
		{Name: "step_100qps_160ws", TargetQPS: 100, TargetWS: 160, Concurrency: 100, Duration: 3 * time.Minute, RequestMix: "auction_mix"},
	...
	return Config{
		...
		WSConnectMaxAttempts: envInt("PERF_WS_CONNECT_MAX_ATTEMPTS", 320),
...
func reconcile(ctx context.Context, cfg Config, data *TestData) string {
	...
	return fmt.Sprintf("item_detail=%s ranking=%s room=%s ws_connected=%d bid_attempts=%d", itemOK, rankingOK, roomOK, wsConnectedCount(data), bidSeq.Load())
}
