...
func buildRequest(cfg Config, data *TestData, stage StageConfig, workerID int, n uint64) RequestSpec {
	...
	if stage.RequestMix == "bid_only" {
		return RequestSpec{Operation: "bid", Method: http.MethodPost, Path: "/api/v1/items/" + url.PathEscape(data.ItemID) + "/bids", Token: data.UserTokens[userIdx]}
	}
	if stage.RequestMix == "item_only" {
		return RequestSpec{Operation: "item_detail", Method: http.MethodGet, Path: "/api/v1/items/" + url.PathEscape(data.ItemID)}
	}
	switch n % 10 {
	case 0, 1, 2, 3, 4, 5, 6, 7:
		return RequestSpec{Operation: "bid", Method: http.MethodPost, Path: "/api/v1/items/" + url.PathEscape(data.ItemID) + "/bids", Token: data.UserTokens[userIdx]}
	case 8:
		return RequestSpec{Operation: "ranking", Method: http.MethodGet, Path: "/api/v1/items/" + url.PathEscape(data.ItemID) + "/ranking?page=1&page_size=20"}
	}
	return RequestSpec{Operation: "item_detail", Method: http.MethodGet, Path: "/api/v1/items/" + url.PathEscape(data.ItemID)}
}
...
func (s *WSStats) deltaSince(snapshot WSStatsSnapshot) (map[string]int64, int64, int64, []time.Duration, []time.Duration, []time.Duration, []time.Duration) {
	if s == nil {
		return map[string]int64{}, 0, 0, nil, nil, nil, nil
	}
	s.mu.Lock()
	defer s.mu.Unlock()
	counts := make(map[string]int64, len(s.eventCounts))
	...
	return counts, timeSyncCount, controlTimeSyncCount, samples, controlArrivalDelaySamples, controlIntervalSamples, readProcessSamples
}
...
func runStage(ctx context.Context, cfg Config, data *TestData, stage StageConfig) StageSummary {
	...
	wsEventCounts, timeSyncCount, controlTimeSyncCount, timeSyncSamples, controlArrivalDelaySamples, controlIntervalSamples, readProcessSamples := data.WSStats.deltaSince(wsSnapshot)
...
func recordEndpointResult(summary *StageSummary, result RequestResult) {
	...
	stats := summary.EndpointStats[operation]
	if stats == nil {
		stats = &EndpointSummary{
			StatusCodes:   map[int]int64{},
			BusinessCodes: map[string]int64{},
		}
		summary.EndpointStats[operation] = stats
	}
	stats.Total++
...
func doRequest(ctx context.Context, cfg Config, data *TestData, stage StageConfig, workerID int, n uint64) RequestResult {
	...
	if spec.Method == http.MethodPost && strings.HasSuffix(spec.Path, "/bids") {
		...
		if err != nil {
			return RequestResult{Operation: spec.Operation, Err: "build_bid_request: " + err.Error()}
		}
		spec = bidSpec
	}
	var body io.Reader
	...
	if err != nil {
		return RequestResult{Operation: spec.Operation, Err: err.Error()}
	}
	if len(spec.Body) > 0 {
	...
	if err != nil {
		return RequestResult{Operation: spec.Operation, Duration: duration, Err: err.Error()}
	}
...
func buildBidRequest(ctx context.Context, cfg Config, data *TestData, stage StageConfig, workerID int, n uint64) (RequestSpec, error) {
	...
	return RequestSpec{Operation: "bid", Method: http.MethodPost, Path: "/api/v1/items/" + url.PathEscape(data.ItemID) + "/bids", Token: data.UserTokens[userIdx], Body: body}, nil
}
...
func defaultPrometheusQueries() []prometheusQuery {
	return []prometheusQuery{
		...
		{
			Name:  "server_http_p95_bid",
			Query: `histogram_quantile(0.95, sum(rate(http_server_request_duration_bucket{http_route="/api/v1/items/{item_id}/bids"}[1m])) by (le))`,
		},
		{
			Name:  "server_http_p99_bid",
			Query: `histogram_quantile(0.99, sum(rate(http_server_request_duration_bucket{http_route="/api/v1/items/{item_id}/bids"}[1m])) by (le))`,
		},
		{
			Name:  "server_http_rps_bid",
			Query: `sum(rate(http_server_request_count_total{http_route="/api/v1/items/{item_id}/bids"}[1m]))`,
		},
		{
			Name:  "server_http_p95_ranking",
			Query: `histogram_quantile(0.95, sum(rate(http_server_request_duration_bucket{http_route="/api/v1/items/{item_id}/ranking"}[1m])) by (le))`,
		},
		{
			Name:  "server_http_p99_ranking",
			Query: `histogram_quantile(0.99, sum(rate(http_server_request_duration_bucket{http_route="/api/v1/items/{item_id}/ranking"}[1m])) by (le))`,
		},
		{
			Name:  "server_http_rps_ranking",
			Query: `sum(rate(http_server_request_count_total{http_route="/api/v1/items/{item_id}/ranking"}[1m]))`,
		},
		{
			Name:  "server_http_p95_item_detail",
			Query: `histogram_quantile(0.95, sum(rate(http_server_request_duration_bucket{http_route="/api/v1/items/{item_id}"}[1m])) by (le))`,
		},
		{
			Name:  "server_http_p99_item_detail",
			Query: `histogram_quantile(0.99, sum(rate(http_server_request_duration_bucket{http_route="/api/v1/items/{item_id}"}[1m])) by (le))`,
		},
		{
			Name:  "server_http_rps_item_detail",
			Query: `sum(rate(http_server_request_count_total{http_route="/api/v1/items/{item_id}"}[1m]))`,
		},
		{
